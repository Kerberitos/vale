			        ...
			        <tbody>
				        ...
				        <tr class="warning">
				            ...
				            @if(strcmp ($anuncio->estatus_revision,"libre") == 0) 
				            	<td>  <a href="{{ route('admin.revisaranuncio.denunciado', [$anuncio->seccion_title, $anuncio->id]) }}" title="Revisar" class="btn btn-success btn-xs">Revisar</a>  </td>
				            
				            @elseif(strcmp ($anuncio->estatus_revision,"ocupado") == 0 & ($anuncio->admin==\Auth::id() ) )
				            	<td>  <a href="{{ route('admin.revisaranuncio.denunciado', [$anuncio->seccion_title, $anuncio->id]) }}" title="Revisar" class="btn btn-info btn-xs">Retomar <span class="hidden-xs"> revisión<span></a>  </td>
				            @else
